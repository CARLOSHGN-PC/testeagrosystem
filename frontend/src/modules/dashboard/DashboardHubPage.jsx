...
import DashboardCTTPage from "./DashboardCTTPage";

const DASHBOARDS = [
  {
    key: "ctt",
    title: "Colheita",
    subtitle: "Acompanhamento de corte, carregamento e transporte da cana-de-açúcar.",
    icon: Tractor,
...
function DashboardCard({ item, onOpen }) {
    ...
    >
      ...
      <div className="relative flex h-full flex-col justify-between gap-5">
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-slate-300">
          <span>{item.enabled ? "Abrir dashboard" : "Módulo em preparação"}</span>
...
export default function DashboardHubPage({ companyId, session }) {
  const [selected, setSelected] = useState(null);

  const content = useMemo(() => {
    if (selected === "ctt") {
      return <DashboardCTTPage companyId={companyId} session={session} onBack={() => setSelected(null)} />;
    }
    return null;
    ...
    <div className="min-h-full bg-[#040814] px-4 py-5 text-white sm:px-6 xl:px-8 2xl:px-10">
      <div className="w-full">
        <div className="mb-6 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[#96a0b8]">
              Selecione um módulo para visualizar os indicadores operacionais.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
          {DASHBOARDS.map((item) => (
            <DashboardCard key={item.key} item={item} onOpen={setSelected} />
          ))}
        </div>
      </div>
    </div>
