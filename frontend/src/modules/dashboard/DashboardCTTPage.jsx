...
function AtrFazendaChart({ data = [], tvMode = false }) {
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={chartData} margin={{ top: 24, right: 18, left: 0, bottom: 18 }}>
        <CartesianGrid stroke="#1b2a40" strokeDasharray="4 4" vertical={false} />
        ...
        <YAxis tickLine={false} axisLine={false} tick={{ fill: "#7f8aa3", fontSize: 11 }} tickFormatter={(value) => decimalBR(value, 0)} width={46} />
        <RechartsTooltip content={<DecimalTooltip decimals={2} />} />
        <Bar dataKey="atr" name="ATR" fill="#9f7aea" radius={[7, 7, 0, 0]} barSize={tvMode ? 38 : 24} maxBarSize={tvMode ? 52 : 34}>
          <LabelList dataKey="atr" position="top" formatter={(value) => decimalBR(value, 2)} fill="#dce4f3" fontSize={10} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
...
export default function DashboardCTTPage({ onBack, companyId }) {
  const [activeDashboard, setActiveDashboard] = useState("entrada");
  ...
  const loadSummary = useCallback(async () => {
    if (!companyId || requestInFlightRef.current) return;
  ...
  }, [companyId, safra, frente, descricao, dataInicio, dataFim]);
      ...
      <div className="relative px-2 py-2 sm:px-3 xl:px-3 2xl:px-4">
        <div className="w-full">
          ...
          <div className="mb-2 flex flex-wrap gap-2 rounded-2xl border border-[#223149]/70 bg-[#07101d]/80 p-2">
            <button type="button" onClick={() => setActiveDashboard("entrada")} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${activeDashboard === "entrada" ? "bg-[#d8ba61] text-black" : "text-[#aab5cd] hover:bg-white/5 hover:text-white"}`}>Entrada de Cana</button>
            <button type="button" onClick={() => setActiveDashboard("fechamento")} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${activeDashboard === "fechamento" ? "bg-[#d8ba61] text-black" : "text-[#aab5cd] hover:bg-white/5 hover:text-white"}`}>Talhões Fechados</button>
          </div>
