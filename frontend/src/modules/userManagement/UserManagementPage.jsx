...
export default function UserManagementPage({ session }) {
  ...
  const resetPassword = async (user) => {
    const result = await showConfirm('Gerar link de redefinição', `Deseja gerar link de reset para ${user.email}?`, 'Gerar link');
    if (!result.isConfirmed) return;

    try {
      const response = await userManagementService.resetPassword(user.uid, user.email);
      showSuccess('Link gerado', `Link de redefinição criado para ${response.data?.email || user.email}.`);
    } catch (error) {
