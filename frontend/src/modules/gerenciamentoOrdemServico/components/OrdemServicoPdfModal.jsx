...
export default function OrdemServicoPdfModal({ isOpen, onClose, ordem, companyId }) {
  ...
  const handleDownload = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${BASE_URL}/api/estimativas/${companyId || ordem.companyId}/relatorios/ordem-servico/pdf?ordemId=${encodeURIComponent(ordem.id || '')}`, {
        method: 'POST',
      ...
      link.href = url;
      link.setAttribute('download', `OrdemServico_${ordem.numeroEmpresa || ordem.sequencial}.pdf`);
      ...
      console.error('Erro ao gerar PDF da Ordem de Serviço:', err);
    ...
    <div className="fixed inset-0 z-50 flex justify-center items-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="rounded-2xl w-full max-w-sm shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200 border" style={{ background: '#111a2d', borderColor: 'rgba(255,255,255,0.12)' }}>
        ...
        <div className="p-6 text-center flex flex-col items-center gap-4">
          ...
          <div>
            <h3 className="text-base font-bold text-white">OS-{ordem.sequencial}</h3>
            <p className="text-sm mt-1" style={{ color: '#8b9bb4' }}>Deseja gerar o documento em PDF desta ordem de serviço?</p>
          </div>
          ...
          <div className="rounded-xl p-4 w-full text-left text-sm mt-2 border" style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.08)' }}>
            <p className="flex justify-between mb-1"><span style={{ color: '#8b9bb4' }}>OS #:</span> <span className="font-mono font-medium text-white">OS-{ordem.sequencial}</span></p>
            <p className="flex justify-between mb-1"><span style={{ color: '#8b9bb4' }}>Operação:</span> <span className="font-medium text-white">{ordem.operacao?.deOperacao || ordem.operacao?.de0peracao || '-'}</span></p>
            <p className="flex justify-between mb-1"><span style={{ color: '#8b9bb4' }}>Resp:</span> <span className="font-medium text-white">{ordem.nomeColaborador || ordem.createdBy || '-'}</span></p>
            <p className="flex justify-between"><span style={{ color: '#8b9bb4' }}>Talhões:</span> <span className="font-medium text-white">{(ordem.talhoes || []).length}</span></p>
          </div>
