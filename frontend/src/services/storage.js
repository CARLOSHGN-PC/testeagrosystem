...
export const syncAllMapLayers = async (companyId, safra = null) => {
  ...
  const modules = ['estimativa', 'ordem-corte', 'planejamento-safra', 'tratos-culturais'];

  for (const activeMapModule of modules) {
    try {
      ...
      console.warn('[syncAllMapLayers] falha em', activeMapModule, e?.message || e);
    }
  }
};
