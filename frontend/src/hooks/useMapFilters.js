...
function isOcOpen(status) {
  const s = normalizeStatusValue(status);
  return ['ABERTA', 'ABERTO', 'OPEN', 'LIBERADA'].includes(s);
}

function isOcWaiting(status) {
  const s = normalizeStatusValue(status);
  return ['AGUARDANDO', 'PENDENTE', 'PENDENTE_APROVACAO', 'PENDENTE APROVACAO', 'RASCUNHO', 'APROVADA'].includes(s);
}

function isOcClosed(status) {
  const s = normalizeStatusValue(status);
  return ['FINALIZADA', 'FECHADA', 'FECHADO', 'EXECUTADA', 'CONCLUIDA', 'CONCLUÍDA'].includes(s);
}
...
function getFeatureIdCandidates(feature) {
  ...
  const candidates = [
    ...
    getUniqueTalhaoId(feature),
    p.FUNDO_AGR !== undefined && p.TALHAO !== undefined ? `${p.FUNDO_AGR}_${p.TALHAO}` : null,
    p.codFaz !== undefined && p.talhao !== undefined ? `${p.codFaz}_${p.talhao}` : null,
  ];
  const result = new Set();
  candidates.forEach((value) => addIdVariant(result, value));
  return Array.from(result).filter(Boolean);
}

function addMapAliases(map, value, payload) {
  if (value === undefined || value === null || value === '') return;
  const aliases = new Set();
  addIdVariant(aliases, value);
  aliases.forEach((alias) => map.set(alias, payload));
  const numeric = Number(String(value).trim());
  if (Number.isFinite(numeric)) map.set(numeric, payload);
}

function featureHasAnyMapValue(feature, mapOrSet) {
  if (!feature || !mapOrSet) return false;
  return getFeatureIdCandidates(feature).some((id) => {
    if (mapOrSet.has(id)) return true;
    const numeric = Number(id);
    return Number.isFinite(numeric) && mapOrSet.has(numeric);
  });
}

function getMapValueForFeature(feature, map) {
  if (!feature || !map) return null;
  for (const id of getFeatureIdCandidates(feature)) {
    if (map.has(id)) return map.get(id);
    const numeric = Number(id);
    if (Number.isFinite(numeric) && map.has(numeric)) return map.get(numeric);
  }
  return null;
}

function hasLocalEstimateFallback(feature, dbTalhao = {}) {
  const p = feature?.properties || {};
  return [
    p._is_estimated,
    p.TCH_EST,
    ...
    dbTalhao.TON_EST,
    dbTalhao.toneladas,
  ].some((value) => value === true || isPositiveNumberLike(value));
}
/**
 * useMapFilters.js
 *
...
export function useMapFilters(geoJsonData, allEstimates, activeMapModule = "estimativa", idsOcultosSet = new Set(), idsAbertosSet = new Set(), currentCompanyId = null, currentSafra = null) {
  ...
  const [ordensCorteStatusMap, setOrdensCorteStatusMap] = useState(new Map());
      ...
      try {
        ...
        talhoes.forEach(t => {
          ...
          [
            ...
            t.TALHAO,
            parsedTalhao,
            `${parsedCodFaz}_${parsedTalhao}`,
            rawCodFaz && rawTalhao ? `${rawCodFaz}_${rawTalhao}` : null,
          ].forEach((value) => addIdVariant(aliases, value));
          aliases.forEach((alias) => talhoesMap.set(alias, t));
        });
        ...
        if (isMounted) {
            ...
            planejamento.forEach(p => {
                if (String(p.statusPlanejamento || '').toLowerCase() !== 'inativo') {
                    addPlanAlias(p.talhaoId, p);
                    addPlanAlias(p.fieldId, p);
                    addPlanAlias(p.featureId, p);
                    addPlanAlias(p.id, p);
                    if (p.fazendaId && (p.talhao || p.talhaoCodigo || p.talhaoId)) {
                      addPlanAlias(`${p.fazendaId}_${p.talhao || p.talhaoCodigo || p.talhaoId}`, p);
                    }
                    if (p.id_fazenda && (p.talhao || p.talhaoCodigo || p.talhaoId)) {
                      addPlanAlias(`${p.id_fazenda}_${p.talhao || p.talhaoCodigo || p.talhaoId}`, p);
                    }
                }
            });
        ...
        if (isMounted) {
          ...
          vinculos.forEach((v) => {
            ...
            const talhaoId = String(v?.talhaoId || v?.fieldId || v?.featureId || '').trim();
            ...
            [
              ...
              v?.talhao,
              v?.TALHAO,
              v?.fazendaId && (v?.talhao || v?.talhaoCodigo || v?.talhaoId) ? `${v.fazendaId}_${v.talhao || v.talhaoCodigo || v.talhaoId}` : null,
              v?.id_fazenda && (v?.talhao || v?.talhaoCodigo || v?.talhaoId) ? `${v.id_fazenda}_${v.talhao || v.talhaoCodigo || v.talhaoId}` : null,
            ].forEach((value) => addIdVariant(aliases, value));
            aliases.forEach((alias) => talhoesByOrdem.get(ordemId).add(alias));

            const ordem = ordemById.get(ordemId) || {};
            const frenteServico = String(v?.frenteServico || ordem?.frenteServico || '').trim();
            const statusEfetivo = v?.status || ordem?.status || '';
            const atual = getMapValueForFeature({ id: talhaoId, properties: { talhaoId } }, frenteByTalhao) || frenteByTalhao.get(talhaoId);
            const proximaPrioridade = prioridadeStatus(statusEfetivo);
            const prioridadeAtual = atual ? prioridadeStatus(atual.status) : -1;

            if (!atual || proximaPrioridade > prioridadeAtual) {
              const payload = { frenteServico, status: statusEfetivo, ordemCorteId: ordemId };
              aliases.forEach((alias) => {
                frenteByTalhao.set(alias, payload);
                statusByTalhao.set(alias, statusEfetivo);
              });
              const numeric = Number(talhaoId);
              if (Number.isFinite(numeric)) {
                frenteByTalhao.set(numeric, payload);
                statusByTalhao.set(numeric, statusEfetivo);
              }
            }
          });
          ...
          setOrdensCorteStatusMap(statusByTalhao);
  ...
  const mappedFeatures = useMemo(() => {
    ...
    return features
      ...
      const dbTalhao = getMapValueForFeature(feature, dbTalhoesMap) || dbTalhoesMap.get(compositeKey) || {};
      const normalizedCorte = normalizeCorte(p.ECORTE || p.corte || dbTalhao.ECORTE || dbTalhao.corte || dbTalhao.CORTE);
      const isEstimated = featureHasEstimate(feature, allEstimatesSet) || hasLocalEstimateFallback(feature, dbTalhao);
      const planejamento = getMapValueForFeature(feature, planejamentoMap) || planejamentoMap.get(uniqueTalhaoId) || planejamentoMap.get(compositeKey);
      ...
      const statusOrdemLocal = getMapValueForFeature(feature, ordensCorteStatusMap) || '';
      const isClosed = featureHasAnyMapValue(feature, idsOcultosSet) || isOcClosed(statusOrdemLocal);
      const isOpen = (featureHasAnyMapValue(feature, idsAbertosSet) || isOcOpen(statusOrdemLocal)) && !isClosed;
      const isWaiting = isOcWaiting(statusOrdemLocal);
      let osStatus = "Aguardando";
      if (isClosed) osStatus = "Fechada";
      else if (isOpen) osStatus = "Aberta";

      const frenteOrdemCorteInfo = getMapValueForFeature(feature, ordensCorteFrenteMap) || null;
      ...
      return {
        ...
        properties: {
          ...
          _os_status: osStatus,
          _has_open_ordem: Boolean(p._has_open_ordem) || isOpen,
          _is_aguardando_ordem: Boolean(p._is_aguardando_ordem) || isWaiting,
          _is_closed_ordem: Boolean(p._is_closed_ordem) || isClosed,
