...
router.post('/colheita/fechamento-oc/import-chunk', requireWriteAccess('dados_dashboard_talhoes_fechados'), async (req, res) => {
  try {
    const companyId = resolveCompanyId(req, req.body);
...
router.get('/colheita/fechamento-oc/historico-producao-ano-anterior', requireModuleAccess('dashboard_talhoes_fechados'), async (req, res) => {
  try {
...
router.get('/colheita/fechamento-oc/dashboard', requireModuleAccess('dashboard_talhoes_fechados'), async (req, res) => {
  try {
    const companyId = resolveCompanyId(req, req.query);
...
router.get('/dashboard/fechamento-oc', requireModuleAccess('dashboard_talhoes_fechados'), async (req, res) => {
  try {
    const companyId = resolveCompanyId(req, req.query);
    const anoParam = Number(req.query.anoRef || req.query.ano || 0);
    const anoRef = Number.isInteger(anoParam) && anoParam >= 1900 && anoParam <= 2500 ? anoParam : historicoAnoAnterior();
...
router.get('/dashboard/fechamento-oc/observacoes', requireModuleAccess('dashboard_talhoes_fechados'), async (req, res) => {
  try {
    const companyId = resolveCompanyId(req, req.query);
...
router.post('/dashboard/fechamento-oc/observacoes', requireWriteAccess('dashboard_talhoes_fechados'), express.json({ limit: '2mb' }), async (req, res) => {
  try {
