...
function featureHasAnyId(feature, setOrMap) {
    if (!setOrMap || (setOrMap.size === undefined && !setOrMap.has)) return false;
    const p = feature.properties || {};
    ...
    for (const value of candidates) {
        ...
        const normId = normalizeId(text);

        if (setOrMap.has(text)) return setOrMap.get ? setOrMap.get(text) : true;
        if (setOrMap.has(text.toUpperCase())) return setOrMap.get ? setOrMap.get(text.toUpperCase()) : true;
        if (setOrMap.has(comparable)) return setOrMap.get ? setOrMap.get(comparable) : true;
        if (setOrMap.has(normId)) return setOrMap.get ? setOrMap.get(normId) : true;
    }
    return false;
}
...
router.get('/talhoes', async (req, res, next) => {
    try {
        ...
        const projectedFeatures = features.map((feature, i) => {
            ...
            const isEstimated = shouldProject ? featureHasAnyId(feature, estimatedDataMap) : Boolean(feature.properties?._is_estimated);
            const estData = shouldProject && isEstimated ? featureHasAnyId(feature, estimatedDataMap) : null;
            ...
            const ordemId = shouldProject ? Array.from(ordemState.idsByOrdem.entries()).find(([oid, idSet]) => featureHasAnyId(feature, idSet))?.[0] || '' : '';
            ...
            const area = estData && estData !== true && estData.area !== undefined ? estData.area : Number(feature.properties?.AREA) || 0;
            const estimatedTon = estData && estData !== true && estData.estimatedTon !== undefined ? estData.estimatedTon : 0;
            const estimatedTch = estData && estData !== true && estData.estimatedTch !== undefined ? estData.estimatedTch : 0;
