...
async function buildFechamentoOcAggregation(rows = [], { companyId, periodo = {}, atrTchConfig = DEFAULT_OC_ATR_TCH_CONFIG, areaPlanejadaPorCorte = {}, areaPlanejadaPorVariedade = {}, planejamentoMeta = {}, premissas = {} } = {}) {
  ...
  // ATR Previsto do Fechamento OC vem das Premissas > Colheita por mês de encerramento.
  // Quando o período possui vários meses, o acumulado fica ponderado por PROD. PREV. de cada mês.
  const atrPrevistoPremissaRow = (row) => atrMetaPremissa(monthName(row?.encerramento));
  ...
  const addRow = (bucket, row) => {
    ...
    const atrPrevistoPremissa = atrPrevistoPremissaRow(row);
    ...
    if (prodPrev > 0 && atrPrevistoPremissa > 0) {
      bucket.atrPrevNumerator += prodPrev * atrPrevistoPremissa;
  ...
  const detalhe = safeRows.map((row) => {
    ...
    const prodReal = parseNumber(row.prodReal);
    const atr = parseNumber(row.atr);
    const atrPrev = atrPrevistoPremissaRow(row);
    ...
    return {
      ...
      tchPrev, tchReal, tch: tchReal, prev: tchPrev, gapPct: gapPctOc(tchReal, tchPrev), atr, atrPrev,
  ...
  const legacyRows = safeRows.map((row) => {
    ...
    return {
      ...
      atrPrev,
      atrPrevNumerator: prodPrev > 0 && atrPrev > 0 ? prodPrev * atrPrev : 0,
      atrPrevWeight: prodPrev > 0 && atrPrev > 0 ? prodPrev : 0,
