...
function drawBars(doc,x,y,w,h,data,labelKey,valueKey,opts={}){
  ...
  const max = Math.max(opts.meta || 0, ...values, 1) * 1.12;
...
export async function gerarDashboardCttPdf(companyId, filters = {}) {
  ...
  drawTwoChartsPage(doc,filters,'ATR','Gráfico individual em página única',
    ...
    (x,y,w,h)=>drawBars(doc,x,y,w,h,summary.atrFazendaData||[],'fazenda','atr',{title:'ATR Fazenda Dia',subtitle:'ATR direto do laboratório',color:COLORS.purple,decimals:2,labelMax:13}));
